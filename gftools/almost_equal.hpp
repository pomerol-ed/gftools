...
/// TODO : replace complex conversion comparison
template <typename T> struct is_number;

// implementations
template <typename T1,typename T2>
    bool almost_equal(T1 t1, T2 t2, real_type tol) {
    return almost_equal_checker<T1,T2>::is_equal(t1,t2,tol); 
}


// TODO : remove complex_type conversion, use a compile time check only
